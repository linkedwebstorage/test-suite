...
`));
  ...
  async run() {
    ...
    try {
      ...
      // Filter tests by level, then by the categories this subject supports.
      const filteredTests = this.filterByCategory(this.filterTests(tests), config);

      if (filteredTests.length < tests.length) {
        const why = [this.options.level && `level: ${this.options.level}`, 'subject features'].filter(Boolean).join(', ');
        console.log(chalk.yellow(`⚡ Running ${chalk.bold(filteredTests.length)}/${tests.length} tests (${why})\n`));
      }
  ...
  // A subject's config.features maps category → boolean. A category absent from
  // the map defaults to supported (so a minimal config runs everything).
  filterByCategory(tests, config) {
    const features = config.features || {};
    return tests.filter((t) => features[t.category] !== false);
  }
  ...
  async generateReports(results, config) {
    ...
    for (const d of ['earl', 'html', 'json']) {
      mkdirSync(resolve(process.cwd(), 'reports', d), { recursive: true });
    }
